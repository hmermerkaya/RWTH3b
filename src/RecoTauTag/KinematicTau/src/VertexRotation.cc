...
*/
bool VertexRotation::tryCorrection(reco::Vertex & pVtx, TransientVertex & sVtx, double & theta, TVector3 & tauFlghtDir, bool forceRotation){
...
*/
void VertexRotation::dumpEvt(const TVector3 & pv, const TVector3 & sv, double thetaMax){
  TVector3 pvE(0,0,0);
  TVector3 svE(0,0,0);
  dumpEvt(pv, pvE, sv, svE, thetaMax);
}

void VertexRotation::dumpEvt(const reco::Vertex & pVtx, const TransientVertex & sVtx, double thetaMax){
  TVector3 pv(pVtx.x(), pVtx.y(), pVtx.z());
  TVector3 sv(sVtx.position().x(), sVtx.position().y(), sVtx.position().z());
  TVector3 pvE(pVtx.xError(), pVtx.yError(), pVtx.zError());
...
void VertexRotation::dumpEvt(const TVector3 & pv, const TVector3 & pvE, const TVector3 & sv, const TVector3 & svE, double thetaMax){
