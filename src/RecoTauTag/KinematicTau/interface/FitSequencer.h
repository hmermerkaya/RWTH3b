...
public:
  ...
  virtual float ndf(){ double x=0; for(unsigned int i=0;i<NDF_.size() ;i++)  x+=NDF_.at(i);   return x;}
  virtual float chi2(){ double x=0; for(unsigned int i=0;i<Chi2_.size();i++)  x+=Chi2_.at(i);  return x;}
